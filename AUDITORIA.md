...
**Ronda v67.22.0:** «elimina estos 4» — Tyranitar, Umbreon, Venusaur y Zekrom (los cuatro que cerraban la galería) se retiran igual que los anteriores: fuera de `assets/avatars/`, de `AVATAR_PACK` (18 criaturas), del precache del Service Worker y de los iconos del lanzador (23 → 19 alias; las tres listas, sincronizadas). Quien tuviera uno elegido vuelve al avatar de letra y al dragón. Y de regalo, un fallo que no era de la app sino de las pruebas: la entrega de la Agenda en `smoke.test.js` llevaba fechas fijas (se abría el 14 y se entregaba el 21 de septiembre), así que el aviso de la víspera («Entrega mañana») dejaba de programarse en cuanto llegaba el día señalado — la prueba caducó sola el 21/09/2026 sin que nadie tocara nada (y el «plazo imposible» del 30/09 iba camino de lo mismo). Ahora las fechas del trabajo son relativas a hoy (abre mañana, se entrega en ocho días), y la comprobación del texto largo del día se calcula con el mismo formato de la app (769 pruebas ✓).
**Ronda v67.21.0:** «elimina las cuatro últimas que se han puesto en la app» — Snivy, Cyndaquil, Froakie y Oshawott se retiran del todo: fuera de `assets/avatars/` (y de `image-search/`, sus originales), de `AVATAR_PACK` (la galería de Ajustes vuelve a las 22 criaturas), del precache del Service Worker y de los iconos del lanzador (las tres listas sincronizadas bajaran de 27 a 23 alias). Quien tuviera uno de los cuatro como avatar o icono de la app no pierde nada: el avatar vuelve solo al de la letra y el icono cae al dragón. La calculadora de faltas de la v67.20 se queda como está (769 pruebas ✓).
